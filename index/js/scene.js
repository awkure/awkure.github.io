if (!(Detector.webgl || Detector.webgl2)) {
    ...
    const cameraViews = [50, 60, 120, 60, 50, 30];
    let cameraIndex = 0;
    ...
    function createBox(parameters) {
        ...
        const boxAspect = parameters.boxAspect;
        ...
        const planeGeo = new THREE.SphereGeometry(boxSize * 1, 32, 16, Math.PI * 0.0, Math.PI * 1, Math.PI * 0.25, Math.PI * 0.5);


        planeRight            = new THREE.Mesh(planeGeo, rightMaterial);
        planeRight.position.x = boxSize * 0.5;
        planeRight.position.y = boxSize * 0.65;
        planeRight.rotation.y = -Math.PI / 2;
        planeRight.scale.z    = 0.3;
        planeRight.position.x = boxSize * 0.65;
        root.add(planeRight);
    ...
    function addHuman() {
        ...
        loader.load("index/models/idle.js", (geometries, materials) => {
            ...
            const animation = geometries[0].animations[0];
    ...
    function addDebris(numTriangles) {
        ...
        for (let i = 0; i < positions.length; i += 9) {
            ...
            const r  = Math.random();
            const fi = Math.random() * 6.283185307179586;
    ...
    function setBoxMaterials(index) {
        ...
        const mapMirrorBack   = cache[backURL];
